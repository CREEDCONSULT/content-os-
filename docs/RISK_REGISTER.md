...
|---|---|---:|---:|---|---|
| R-001 | MVP breadth produces disconnected placeholder screens | High | High | Deliver connected vertical slices and label deferred integration states honestly | Controlled for Milestones 0-3; active for remaining scope |
...
| R-003 | Secrets enter Git, logs, model prompts, or vault | Medium | Critical | `.env` exclusion, server-only adapters, secret scanning, redacted integration status | Controlled locally; recheck every commit |
...
| R-006 | Database and vault diverge | Medium | High | Version IDs, hashes, provenance, conflict records, reconciliation tests | Planned |
| R-007 | Heartbeat duplicates runs or spends beyond budget | Medium | High | Idempotency keys, database locks, per-run budgets, mock default | Planned |
| R-008 | Creator benchmarking becomes copying | Medium | High | Evidence levels, transferable/non-transferable split, originality checks | Planned |
| R-009 | Financial content implies advice or certainty | Medium | Critical | Risk classification, fact-check state, blocked claims, approval gate | Planned |
